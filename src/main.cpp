...
unsigned long lastTime = 0;     // To track the last time we printed the count
unsigned long count = 0;        // To count how many times the function was called
...
const unsigned int updateInt = 10; // how fast to do an update in the loop, 40 should be 10hz
...
void setup(void) {
  ...
  if (sendToEsp) { delay(100); HWSERIAL2.begin(500000); }
  delay(100);
  tft.begin();
  delay(100);
  //tft.setRotation(2);
